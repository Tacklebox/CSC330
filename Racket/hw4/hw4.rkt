...
(define (stream-for-n-steps s n)
  (cond
    [(= n 0) null]
    [(> n 0) (let ([stream-item (s)])
               (cons
                 (car stream-item)
                 (stream-for-n-steps (cdr stream-item) (- n 1)))
               )]
  ...
  (letrec
    ([funny-number-stream-helper (lambda (n)
                                   (cons
                                     ...
                                     (thunk
                                       (funny-number-stream-helper (+ 1 n)))
                                     ))])
...
(define (stream-add-zero s)
  (let
    ([stream-item (s)])
    (thunk
      (cons (cons 0 (car stream-item)) (stream-add-zero (cdr stream-item))))
    ))
    ...
    ([vector-assoc-helper
       (lambda (index)
         (cond
           ...
           [(not (pair? (vector-ref vec index)))
            (vector-assoc-helper (+ 1 index))]
    ...
    ([cache-pointer 0]
     ...
     [cache! (lambda (result)
               (begin
                 ...
                 (set! cache-pointer (+ 1 cache-pointer)))
               )])
    (lambda (v)
      (let ([from-cache (vector-assoc v cache)]
            [from-xs (delay (assoc v xs))])
